...
if __name__ == '__main__':
    ...
    train = MINDDataset(path.join(DATA_SIZE,'train/news.tsv'), path.join(DATA_SIZE,'train/behaviors.tsv'), path.join(DATA_SIZE,'train/all_embeddings.vec'), batch_size=BATCH_SIZE, model=args.pretrained_model)
    train.load_data()
    ...
    valid = MINDDataset(path.join(DATA_SIZE,'valid/news.tsv'), path.join(DATA_SIZE,'valid/behaviors.tsv'), path.join(DATA_SIZE,'all_embeddings.vec'), batch_size=BATCH_SIZE, model=args.pretrained_model, subset='valid')
    valid.load_data()
